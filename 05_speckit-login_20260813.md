...
> 1차시(SDD 방법론, `01_speckit_sdd_methodology_20260813.md`) → 2차시(Spec Kit 가이드, `02_speckit-guide_20260813.md`) → 3차시(Gemini CLI 실습) → 4차시(이 문서)로 이어지는 강의이며  
> **도구는 GitHub Copilot Free 를 사용**하여 SDD 방법론이 어떻게 동작하는지 실습한다.
>
> 2차시에서 배운 `/speckit.*` 명령어(constitution ~ implement, checklist)의 **역할과 순서**는 이미 알고 있다는 전제로 진행한다. 명령어 자체의 개념 설명이 필요하면 2차시 자료의 「6. 핵심 워크플로 명령어」·「7. checklist」·「8. implement 후 테스트 전략」을 먼저 참고한다.
...
15. [Part 11. 심화 확장 아이디어 — 실전 백엔드 연동](#part-11-심화-확장-아이디어--실전-백엔드-연동)
16. [3차시(Gemini)와의 차이점 요약](#3차시gemini와의-차이점-요약)
17. [트러블슈팅](#트러블슈팅)
18. [전체 명령어 요약표](#전체-명령어-요약표)
...
```
/speckit-clarify 
specs\002-login-screen\spec.md 에 아래 내용을 보강해 줘.
1. http://localhost:3000 최초에 접근하면 로그인화면이 표시된다.
2. 로그인이 성공하면 todolist 화면이 표시된다.
```
---
...
## Part 11. 심화 확장 아이디어 — 실전 백엔드 연동

이번 실습(001~003)은 학습 목적상 **백엔드 없이 브라우저 안에서만 동작**하도록 constitution.md에 "HTML/CSS/JS만 사용, 외부 프레임워크 금지" 원칙을 명시했다. 아이디/비밀번호가 `admin`/`1234`로 코드에 하드코딩되어 있는 것도 이 때문이다.

실제 서비스라면 이 값들은 서버와 데이터베이스에서 검증해야 한다. **SDD의 핵심은 spec.md의 수용 기준(Given-When-Then)은 거의 그대로 두고, constitution.md의 기술 원칙만 바꿔서 plan.md 이후 단계를 다시 생성하는 것**이다. 즉, "무엇을(What)"은 유지한 채 "어떻게(How)"만 교체한다.
...
[확장 후 constitution.md — /speckit.constitution 재실행]
- Backend: FastAPI (Python)
- 인증 방식: JWT (Access Token + Refresh Token)
- 비밀번호는 bcrypt로 해싱하여 저장 (평문 비교 금지)
- 로그인 실패 횟수는 Redis에 저장 (TTL 30분)
```

```
[다시 실행할 명령어]
/speckit.plan       → FastAPI + JWT + bcrypt 기준으로 plan.md 재생성
/speckit.analyze    → spec.md(수용 기준)와 새 plan.md의 일치 여부 확인
/speckit.tasks      → 재생성된 plan.md 기준 tasks.md 재생성
/speckit.implement  → 실제 로그인 API(Python) 코드 생성
```

> **spec.md는 그대로다** — "아이디/비밀번호가 비어 있으면 오류 메시지" 같은 수용 기준은 프론트엔드든 백엔드든 동일하게 적용된다. 바뀌는 것은 `plan.md`(어떻게 만들지)와 그 이후 산출물뿐이다. 이것이 1차시에서 배운 **"사양이 진실의 근원"** 원칙이 실전에서 어떻게 작동하는지 보여주는 가장 확실한 사례다.
>
> 이 확장을 직접 해보고 싶다면 2차시 자료(`02_speckit-guide_20260813.md`)의 「9. 실습 — 로그인 화면 만들기」에 있는 FastAPI + JWT + Redis 기반 constitution.md·plan.md·구현 코드 예시를 참고한다.
...
*GitHub Copilot Free 무료 한도 기준: 월 50회 채팅, 2,000회 코드 완성 (2026년 6월 기준, 변경될 수 있으므로 실습 전 GitHub 공식 페이지에서 최신 한도를 확인할 것)*
