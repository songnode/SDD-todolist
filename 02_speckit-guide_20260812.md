...
> **1차시 복습**: SDD의 4단계(Specify → Plan → Task → Implement) 이론과 스펙 설계 전략(Less is More, 계층적 스펙 구조)을 학습했습니다. 이론 전체는 1차시 자료 「SDD(Specification-Driven Development) 방법론」을 참고하세요.  
> 이번 시간에는 GitHub Spec Kit 도구로 이론을 직접 실습합니다.
...
### Step 0: 프로젝트 헌법 설정 — `/speckit.constitution`

> **12절 FAQ에서 강조한 원칙**: constitution.md는 프로젝트 시작 직후, 첫 기능 명세(`/speckit.specify`) **실행 전**에 만들어야 합니다. 이번 실습에서도 로그인 기능 명세를 만들기 전에 먼저 프로젝트 헌법을 세웁니다.

```
[사용자 입력]
/speckit.constitution
우리 프로젝트의 기술 원칙을 정의해줘.
- Backend: FastAPI (Python)
- Database: Azure SQL Database
- 인증: JWT 기반
- 모든 기능은 단위 테스트를 함께 작성 (커버리지 80% 이상)
- 비밀번호 등 민감 정보는 반드시 암호화하여 저장
```

**AI가 생성하는 `.specify/memory/constitution.md`**:

```yaml
project_principles:
  architecture:
    - "Backend: FastAPI (Python), Frontend: React + TypeScript"
    - "Database: Azure SQL Database"
  ...
  change_management:
    - "API 변경 시 반드시 spec.md 먼저 업데이트"
```

> **이 단계를 건너뛰면 생기는 문제**: constitution.md 없이 바로 `/speckit.specify`를 실행하면, AI가 기능마다 다른 데이터베이스나 인증 방식을 임의로 선택할 수 있습니다. Step 3(`/speckit.plan`)에서 "constitution.md 반영"이라고 표시되는 항목들은 모두 이 단계에서 정의한 원칙입니다.
...
- 1차시 자료: 「SDD(Specification-Driven Development) 방법론」(01_speckit_sdd_methodology_20260812.md)
